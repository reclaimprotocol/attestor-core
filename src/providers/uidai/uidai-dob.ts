/**
 * Verify the dob from aadhar card
 */
...
const UidaiAadhaarDOB: Provider<UidaiParams, UidaiSecretParams> = {
	hostPort: HOSTPORT,
	areValidParams(params): params is UidaiParams {
		return typeof params.dob === 'string'
	},
	...
	assertValidProviderReceipt(receipt, { dob }) {
		...
		if(!json.responseData.dob) {
			throw new Error('No DOB was found in the response data!')
		}

		if(json.responseData.dob !== dob) {
			throw new Error(
				`Invalid DOB: DOB doesn't match fetched DOB ${json.responseData.dob}`
			)
		}
	},
}

export default UidaiAadhaarDOB