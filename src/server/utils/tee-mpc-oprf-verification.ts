/**
 * TEE MPC OPRF Verification
 * Verifies MPC OPRF outputs from TEE_K and TEE_T match
 *
 * Unlike ZK OPRF which requires proof verification, MPC OPRF outputs
 ...
 */
export function verifyMpcOprfOutputs(
	...
	// Empty is valid - no MPC OPRF was requested
	if(kOutputs.length === 0 && tOutputs.length === 0) {
		logger.debug('No MPC OPRF outputs to verify')
		return []
	}
	...
	if(kOutputs.length !== tOutputs.length) {
		throw new AttestorError(
			'ERROR_INVALID_CLAIM',
			`MPC OPRF count mismatch: TEE_K has ${kOutputs.length}, TEE_T has ${tOutputs.length}`
		)
	}

	logger.info(`Verifying ${kOutputs.length} MPC OPRF outputs`)
	...
	for(const [i, kOut] of kOutputs.entries()) {
		...
		if(kOut.tlsStart < 0 || tOut.tlsStart < 0) {
			throw new AttestorError(
				'ERROR_INVALID_CLAIM',
				`MPC OPRF invalid position at index ${i}: negative start position`
			)
		}
		...
		if(kOut.tlsLength <= 0 || kOut.tlsLength > 64 || tOut.tlsLength <= 0 || tOut.tlsLength > 64) {
			throw new AttestorError(
				'ERROR_INVALID_CLAIM',
				`MPC OPRF invalid length at index ${i}: must be 1-64 bytes ` +
		...
		if(kOut.hashOutput.length !== 32 || tOut.hashOutput.length !== 32) {
			throw new AttestorError(
				'ERROR_INVALID_CLAIM',
				`MPC OPRF invalid hash size at index ${i}: expected 32 bytes ` +
		...
		if(kOut.tlsStart !== tOut.tlsStart || kOut.tlsLength !== tOut.tlsLength) {
			throw new AttestorError(
				'ERROR_INVALID_CLAIM',
				`MPC OPRF position mismatch at index ${i}: ` +
		...
		if(!buffersEqual(kOut.hashOutput, tOut.hashOutput)) {
			throw new AttestorError(
				'ERROR_INVALID_CLAIM',
				`MPC OPRF hash mismatch at index ${i}: outputs differ between TEE_K and TEE_T`
			)
		}
	...
	logger.info(`Successfully verified ${results.length} MPC OPRF outputs`)
	return results
}
