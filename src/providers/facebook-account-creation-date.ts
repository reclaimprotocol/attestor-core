...
const facebookAccountCreationDate: Provider<
  FacebookAccountCreationDateParams,
  FacebookAccountCreationDateSecretParams
> = {
	hostPort: HOSTPORT,
	areValidParams(params): params is FacebookAccountCreationDateParams {
		return typeof params.joinedAt === 'string'
	},
	...
	assertValidProviderReceipt(receipt, params) {
		...
		if(!match) {
			throw new Error('Could not find joined date')
		}

		const joinedAt = `${match[1]} ${match[2]}`


		if(joinedAt.toLowerCase() !== params.joinedAt.toLowerCase()) {
