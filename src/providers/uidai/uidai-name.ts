/**
 * Verify the name from aadhar card
 */
...
const UidaiAadhaar: Provider<UidaiParams, UidaiSecretParams> = {
	hostPort: HOSTPORT,
	...
	assertValidProviderReceipt(receipt, { name }) {
		...
		if(!json.responseData.name) {
			throw new Error('No name was found in the response data!')
		}

		if(json.responseData.name !== name) {
			throw new Error(
				`Invalid Namw: Name doesn't match fetched name ${json.responseData.name}`
			)
		}
	},
}

export default UidaiAadhaar
