...
// Define the provider
const cartaEsopsCount: Provider<CartaUsernameParams, CartaUsernameSecretParams> = {
	hostPort: HOSTPORT,
	...
	assertValidProviderReceipt(receipt, { count }) {
		...
		if(count !== json.count) {
			throw new Error(`Count "${json.count}" does not match expected "${count}"`)
		}
	},
}
