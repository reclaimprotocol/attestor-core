...
type QuoraUserIdParams = {
	userId: string
}

// params required to generate the http request to Quora
// these would contain fields that are to be hidden from the public,
// including the witness
type QuoraLoginSecretParams = {
...
const quoraUser: Provider<QuoraUserIdParams, QuoraLoginSecretParams> = {
	hostPort: HOSTPORT,
	areValidParams(params): params is QuoraUserIdParams {
		return (
	...
	assertValidProviderReceipt(receipt, { userId }) {
		...
		// quora.com/profile will redirect to quora.com/profile/{{userid}}
		...
		if(res.headers['location'] !== `https://${HOST}${PATH}${userId}`) {
			throw new Error(`Invalid profile redirect https://${HOST}${PATH}${userId}. Found ${res.headers['location']}`)
		}
