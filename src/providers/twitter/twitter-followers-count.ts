...
type TwitterFollowersCountParams = {
	numFollowers: string
}

type TwitterFollowersCountSecretParams = {
	cookieStr: string
}
...
const twitterFollowersCount: Provider<TwitterFollowersCountParams, TwitterFollowersCountSecretParams> = {
	hostPort: HOSTPORT,
	areValidParams(params): params is TwitterFollowersCountParams {
		return typeof params.numFollowers === 'string'
	},
	...
	assertValidProviderReceipt(receipt, { numFollowers }) {
		...
		const reqMatch = `"normal_followers_count":${numFollowers},"pinned_tweet_ids`

		if(!html.includes(reqMatch)) {
			throw new Error('Invalid number of followers')
		}
	},
}

export default twitterFollowersCount
